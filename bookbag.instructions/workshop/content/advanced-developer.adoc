...
[start=1]
. Start by forking the necessary repositories used for this application to make them "your own" - just like you did in the `Junior Developer` exercise. If you don't remember how to do the forking steps, please re-visit the previous exercise to get the steps. The difference here is that the application is based on multiple services to operate correctly - the beginning to a micro services approach - and hence we have a need to use more than one repository. +
 +
