...
[start=9]
. Since this is a new image that we are still trialing to see if it meets out needs, we will just load it up temporarily by using the inline method we used earlier. This will reference the local image and let us test it before we make it permanent by pushing the image to a centralized image registry outside of the OpenShift environment and update the Devfile with the new image. +
 +
